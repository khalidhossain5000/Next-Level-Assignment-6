...
import type {
  IPlannedOutagePayload,
  IPlannedOutageUpdatePayload,
} from "./planned-outage.interface";
...
const createPlannedOutageInDb = async (payload: IPlannedOutagePayload) => {
...
const updatePlannedOutage = async (
  ...
  if (payload.startTime && finalStartTime < new Date()) {
    throw new AppError(
      httpStatus.BAD_REQUEST,
      "Load shedding schedule cannot start in the past"
    );
